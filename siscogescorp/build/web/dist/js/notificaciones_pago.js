 ...
 */
 ...
 function consulta_notificaciones_pagos(){
     
       
     document.getElementById("mi_tabla").innerHTML = "";
    var htmlTable="<table id='consulta_notificaciones_pago' class='table table-striped table-bordered dt-responsive nowrap table-hover' cellspacing='0' width='100%'><thead><tr bgcolor='#FBF5EF'><th class='col-sm-1 text-left' style='color: #3c8dbc'>ID</th><th class='col-sm-1 text-left' style='color: #3c8dbc'>Documento</th><th align='left' class='col-sm-3 text-left' style='color: #3c8dbc'>Descripción</th><th class='col-sm-1 text-right'  style='color: #3c8dbc'>Fecha Creación</th><th align='center' class='col-sm-1 text-right'  style='color: #3c8dbc'>#Deudores</th><th align='center' class='col-sm-1 text-right'  style='color: #3c8dbc'>#Recibidos</th><th align='rigth' class='col-sm-1 text-right'  style='color: #3c8dbc'>Acción</th> </tr> </thead><tbody></tbody></table>";
    document.getElementById("mi_tabla").innerHTML = htmlTable;
                
    var table = $('#consulta_notificaciones_pago').DataTable( {
        "ajax": {   
            "data": {"accion": "consultar"},
            "url": "notificacionespagos",
            "type": "GET"
            },
            "columns": [
                { "data": "id_notificaciones_pago"},
...
function agregar_notificaciones_pago(){
    ...
    if(descripcion===""){
      ...
      $.ajax({
        data:{"accion":"nuevas_notificaciones", "descripcion":descripcion,"id_documento":id_documento},
        url: "notificacionespagos",
        type: 'POST',
        beforeSend: function () {
        },
        ...
        if(!response.toString("La Notificacion de Pago fue eliminada exitosamente")){
            MsgSalidaModalA(response);
        }else{
            MsgSalidaModalA(response);
...
function consulta_mis_documentos(){
          

     document.getElementById("id_documento").innerHTML="";
       $("#id_documento").append($("<option>",{value:"0",text:"Seleccione el documento"}));
        $.getJSON("notificacionespagos", {"accion" : "mis_documentos"}, function(result){
              $.each(result.data, function(key, val){   
                  console.log("val.idDocumento>>"+val.idDocumento+"text:val.nombreDocumento > "+val.nombreDocumento)
               $("#id_documento").append($("<option>",{value:val.idDocumento,text:val.nombreDocumento}));
              });
        });
}